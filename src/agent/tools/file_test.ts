...
Deno.test("ReadFileTool fails on missing file", async () => {
  const result = await reader.execute({ path: "/tmp/denoclaw-nonexistent-file-xyz" });
...
Deno.test("WriteFileTool dry_run by default", async () => {
  const result = await writer.execute({ path: "/tmp/test.txt", content: "hello" });
  assertEquals(result.success, true);
...
Deno.test("WriteFileTool writes with dry_run=false", async () => {
  ...
  const result = await writer.execute({ path: tmp, content: "hello world", dry_run: false });
  assertEquals(result.success, true);
