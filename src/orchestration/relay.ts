...
export class LocalRelay {
  ...
  constructor(config: LocalRelayConfig, deps?: { toolExecution?: ToolExecutionPort }) {
    this.config = config;
  ...
  async connect(): Promise<void> {
    ...
    this.ws.onopen = () => {
      ...
      const registration = buildRelayRegistrationMessage({
        ...
        toolPermissions: this.toolExecution.getAdvertisedToolPermissions(
          this.config.capabilities.tools,
        ),
