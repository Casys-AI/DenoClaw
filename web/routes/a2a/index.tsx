            ...
            <tbody>
              {(() => {
                ...
                for (const [traceId, chainTasks] of chains) {
                  const sorted = chainTasks.sort((a, b) => a.timestamp.localeCompare(b.timestamp));
                  const isActive = sorted.some((t) => t.status === "sent" || t.status === "received");

                  sorted.forEach((task, idx) => {
                    const isFirst = idx === 0;
                    const isLast = idx === sorted.length - 1;
                    const treeChar = sorted.length === 1 ? "─" : isFirst ? "┌" : isLast ? "└" : "├";

                    rows.push(
                      <tr key={task.taskId} class={`hover ${isActive ? "border-l-2 border-primary" : ""}`}>
                        <td class="font-data text-xs text-neutral-content w-24">
                          {isFirst && (
                            <span class="text-primary" title={traceId}>{traceId.slice(0, 8)}</span>
                          )}
                        </td>
                        <td class="font-data text-xs">
                          <span class="text-neutral-content mr-1">{treeChar}</span>
                          <span class="text-primary">{task.taskId.slice(0, 10)}...</span>
                        </td>
                        <td>
                ...
                for (const task of orphans) {
                  rows.push(
                    <tr key={task.taskId} class="hover">
                      <td class="font-data text-xs text-neutral-content">—</td>
                      <td class="font-data text-xs text-primary">{task.taskId.slice(0, 10)}...</td>
                      <td>
                        <span class="font-medium">{task.from}</span>
                        <span class="text-neutral-content mx-1">→</span>
                        <span class="font-medium">{task.to}</span>
