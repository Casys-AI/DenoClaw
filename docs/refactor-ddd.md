...
**Date :** 2026-03-27 **Statut :** Terminé

## Objectif

Passer d'une architecture en 16 slices techniques vers 8 bounded contexts DDD,
avec injection de dépendances et types co-localisés.
...
- Les imports entre domaines passent par le `mod.ts` barrel (sauf shared/ qui
  est direct)
...
- [x] Créer les dossiers cibles (`shared/`, `messaging/`, `orchestration/`,
      `llm/`)
- [x] Snapshot git propre (commit l'état actuel avant refactor)

### Phase 1 : Shared Kernel

- [x] Créer `src/shared/types.ts` — extraire les types cross-domain de
      `src/types.ts`
...
- [x] Créer `src/agent/types.ts` — `AgentConfig`, `AgentResponse`, `Skill`,
      `CronJob`, `AgentDefaults`, `AgentsConfig`, `ToolsConfig`
- [x] Créer `src/agent/tools/types.ts` — `BuiltinToolName`,
...
- [x] Mettre à jour les imports dans `agent/loop.ts`, `agent/runtime.ts`,
      `agent/context.ts`, `agent/skills.ts`
...
- [x] Déplacer `src/providers/manager.ts` → `src/llm/manager.ts` (prend
      `ProvidersConfig` pas `Config`)
...
- [x] Créer `src/messaging/types.ts` — `ChannelMessage`, `Session`, configs
      channels
...
- [x] Déplacer `src/broker/client.ts` → `src/orchestration/client.ts`
...
- [x] Déplacer `src/gateway/mod.ts` → `src/orchestration/gateway.ts` (DI +
      AuthManager + close())
- [x] Déplacer `src/sandbox/mod.ts` → `src/orchestration/sandbox.ts`
      (`SandboxApiConfig`)
- [x] Créer `src/orchestration/mod.ts` (barrel avec `BrokerServerDeps`,
      `GatewayDeps`)
...
- [x] Créer `src/config/types.ts` — `Config` assemble les sub-configs de chaque
      domaine
...
- [x] `SessionManager` : constructeur `(kv?: Deno.Kv)`, supprimer
      `getSessionManager()`
- [x] `ChannelManager` : constructeur `(bus: MessageBus)`, supprimer
      `getChannelManager()`
...
- [x] Supprimer tous les anciens dossiers (utils, types.ts, providers, broker,
...
- [x] Barrels complets : `AgentLoopDeps`, `BrokerServerDeps`, `GatewayDeps`,
      `createDefaultConfig`
...
- Chaque phase se termine par `deno task check` + `deno task test` — on ne passe
  à la suivante que si tout est vert
