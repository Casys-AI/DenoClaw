...
import type { RuntimeTaskContinueMessage, RuntimeTaskSubmitMessage } from "./runtime_transport.ts";
...
class MemoryStub implements MemoryPort {
  ...
  close(): void {}
  addMessage(message: Message): Promise<void> { this.messages.push(message); return Promise.resolve(); }
  getMessages(): Message[] { return [...this.messages]; }
  getRecentMessages(count: number): Message[] { return this.messages.slice(-count); }
  clear(): Promise<void> { this.messages = []; return Promise.resolve(); }
  get count(): number { return this.messages.length; }
  remember(): Promise<void> { return Promise.resolve(); }
  recall(): Promise<[]> { return Promise.resolve([]); }
  listTopics(): Promise<string[]> { return Promise.resolve([]); }
  forgetTopic(): Promise<void> { return Promise.resolve(); }
}
...
function createRuntime(
  ...
  runtimeAny.context = { buildContextMessages: (messages: Message[]) => messages };

  return runtime;
}

Deno.test("AgentRuntime wires llm/tool and canonical task ports independently (nominal)", async () => {
  ...
  const llmPort: AgentLlmToolPort = {
    ...
    execTool: (): Promise<ToolResult> => Promise.reject(new Error("not used")),
    close: () => {},
  };
...
Deno.test("AgentRuntime fails fast when canonical task port implementation is missing", async () => {
  const llmPort: AgentLlmToolPort = {
    startListening: () => Promise.resolve(),
