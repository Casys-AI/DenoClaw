# ADR-008: Deploy Agent Runtime Corrections — Broker Orchestrates, Agents React

**Status:** Accepted **Date:** 2026-03-27

## Context

An audit of the official Deno Deploy agent apps docs revealed fundamental errors
in our execution model. The existing architecture assumed that agent apps could
run as long-lived daemons using `Deno.cron()` and `kv.listenQueue()`. In
practice, neither API works in deployed agent apps.

### Verified claims (official sources)

| Claim                                        | Verdict   | Source                                                                                        |
| -------------------------------------------- | --------- | --------------------------------------------------------------------------------------------- |
| `Deno.cron()` blocked in deployed agent apps | CONFIRMED | docs.deno.com/subhosting/api/ — _"Deno Cron and Queues do not currently work for Subhosting"_ |
...
| Workers inside deployed agent apps           | UNKNOWN   | Not documented and does not solve persistence                                                 |
...
### Critical (broken in deployed agent apps)
...
| `src/cli/setup.ts`             | Deno Deploy API v1 (`api.deno.com/v1`).                 |
...
In deployed agent apps, the agent is a pure HTTP server. It receives work by
POST, performs its computation, including multi-step LLM work, and either
returns a synchronous result (short task) or a `taskId` plus SSE stream (long
task). The Broker is the durable message store and cron dispatcher.
...
| Agent          | Deploy agent app (warm-cached V8 isolate) | **Worker** (`new Worker()`)     |
...
| -------------------- | --------------------------------------- | ----------------------------------- | -------------------------------------- |
| **HTTP POST**        | Wake-up transport for A2A work          | Not required (workers stay alive)   | Only way to wake deployed agent apps   |
...
`BroadcastChannel` is **not** used for inter-agent communication, only for infra
concerns such as shutdown and config reload.
...
- **Private KV** (`./data/<agentId>.db` locally, bound DB in deploy) for memory,
  sessions, and agent-specific A2A history
...
```
Deploy ↔ agent apps = direct HTTP (same Deno platform, no tunnel)
...
Use `@deno/oidc` when possible in deployed agent apps, since they run on the
same platform as Deploy. The Broker verifies `org_id` + `app_id` in the JWT.
...
(tools, auth), VPS/GPU hosts (resources), and other Brokers (federation). Deploy
↔ agent apps does not need a tunnel.

## Consequences

- The architecture moves from "agent orchestrates, Broker routes" to **"Broker
  orchestrates, agent reacts"**
- Agent code is identical in local mode (Worker) and deploy mode (Deploy app)
...
- The three-layer model (Process/Worker/Subprocess ↔ Broker/Agent App/Sandbox)
  becomes coherent
