...
Deux modes : **local** (dev) et **en ligne** (prod). On travaille en local, on publie quand c'est prêt.
Chaque commande est interactive par défaut, AX-compatible via flags (`--json`, `--yes`).
