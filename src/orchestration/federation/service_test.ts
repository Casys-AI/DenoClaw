...
import { signCatalogEnvelope } from "./crypto.ts";
import type {
  CrossBrokerHopEvent,
...
class FlakyRoutingPort implements FederationRoutingPort {
  ...
  async resolveTarget(
    ...
    return { kind: "remote" as const, remoteBrokerId: "broker-b", reason: "test" };
  }
...
Deno.test("FederationService.syncSignedCatalog accepts trusted signed catalogs", async () => {
  const kvPath = await Deno.makeTempFile({ suffix: ".db" });
  const kv = await Deno.openKv(kvPath);
  try {
    const adapter = new KvFederationAdapter(kv);
...
Deno.test("FederationService.syncSignedCatalog rejects invalid signature", async () => {
  const kvPath = await Deno.makeTempFile({ suffix: ".db" });
  const kv = await Deno.openKv(kvPath);
  try {
    const adapter = new KvFederationAdapter(kv);
...
Deno.test("FederationService.evaluateRouteAuthorization distinguishes local and remote denials", async () => {
  const kvPath = await Deno.makeTempFile({ suffix: ".db" });
  const kv = await Deno.openKv(kvPath);
  try {
    const adapter = new KvFederationAdapter(kv);
...
Deno.test("FederationService.forwardTaskIdempotent retries then succeeds", async () => {
  const kvPath = await Deno.makeTempFile({ suffix: ".db" });
  const kv = await Deno.openKv(kvPath);
  try {
    const adapter = new KvFederationAdapter(kv);
...
Deno.test("FederationService.forwardTaskIdempotent moves to dead-letter after max attempts", async () => {
  const kvPath = await Deno.makeTempFile({ suffix: ".db" });
  const kv = await Deno.openKv(kvPath);
  try {
    const adapter = new KvFederationAdapter(kv);
    ...
    assertEquals(observability.events.length, 2);
    assertEquals(observability.events.every((event) => !event.success), true);
  } finally {
    kv.close();
    await Deno.remove(kvPath);
  }
...
Deno.test("FederationService rotates identity key and session via ports", async () => {
  const kvPath = await Deno.makeTempFile({ suffix: ".db" });
  const kv = await Deno.openKv(kvPath);
  try {
    const adapter = new KvFederationAdapter(kv);
    ...
    const identity = await service.rotateIdentityKey("broker-b", "pub-key-v2");
