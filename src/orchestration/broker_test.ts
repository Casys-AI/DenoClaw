...
Deno.test(
  ...
  async () => {
    ...
    try {
      ...
      const grantedResume = bodyBrokerMetadata(updatedPausedAgentTask)
        .pendingResumes?.["git status"];
      assertExists(grantedResume);
      assertEquals(grantedResume.kind, "approval");
      assertEquals(grantedResume.approved, true);
      assertEquals(grantedResume.command, "git status");
      assertEquals(grantedResume.binary, "git");
